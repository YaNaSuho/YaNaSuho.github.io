...
{
    var strength_cat = document.getElementById("strength_cat");
    for(var i=0; i<cat_filters[0].length;i++)
    {
         strength_cat.innerHTML+="<input type=\"checkbox\" class=\"larger\">"+
                "<label for=\"strength\" class = \"filter_label\">"+
                cat_filters[0][i]+"</label><br>";
    }
    var taste_cat = document.getElementById("taste_cat");
    for(var i=0; i<cat_filters[1].length;i++)
    {
        taste_cat.innerHTML+="<input type=\"checkbox\" class=\"larger\">"+
                "<label for=\"strength\" class = \"filter_label\">"+
                cat_filters[1][i]+"</label><br>";
    }
    var base_cat = document.getElementById("base_cat");
    for(var i=0; i<cat_filters[2].length;i++)
    {
        base_cat.innerHTML+="<input type=\"checkbox\" class=\"larger\">"+
                "<label for=\"strength\" class = \"filter_label\">"+
                cat_filters[2][i]+"</label><br>";
    }
   ...
   checkboxes = document.getElementsByClassName('larger');
        ...
        n = cat_filters.length
        filter_data=new Array(n).fill("");
        function get_checks()
        {
            filter_data= new Array(n).fill("");
            var t=0;
            for(var i =0; i<n;i++)
            for(var j =0; j<cat_filters[i].length;j++)
            {
                if(checkboxes[t].checked == true)
                filter_data[i]+=cat_filters[i][j];
    ...
    function generate_catalogue_items()
    {
       ...
       //console.log("generated");
        ...
        for (const element of cocktails_array)
        {
            ...
            "<img alt=\"\" class=\"cocktail_img\" src=\"img/"+element.img_path+"\"></div>"+
            "<div>"+
       ...
       // console.log(element.id);
            ...
            {   id_to_send = element.id;
                order_item.innerHTML="<div class=\"image-box_desktop\"><img alt=\"\" class=\"cocktail_img\" "+"src=\"img/"+element.img_path+"\"></div>"+
            "<div>"+
                "<h2 class=\"common-sub-title title desktop\">"+element.name+"</h2>"+
...
var order_name_input = document.getElementById("order_name_input");
    ...
    filter_btn.onclick = function()
    {
        ...
        else//for order
        {
            ...
            tg.sendData(item);
            //console.log(item);
             //console.log(cocktail.components);
